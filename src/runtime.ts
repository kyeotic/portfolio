...
import { esbuild, ensureEsbuildInitialized, type esbuildWasm } from './esbuild.ts'
...
export function createRequestHandler<Context = unknown>({
  appDirectory = path.resolve(cwd(), 'app'),
  assetsDirectory = path.resolve(cwd(), 'assets'),
  generatedFile = path.resolve(cwd(), 'remix.gen.ts'),
  browserImportMapPath,
  staticDirectory = path.resolve(cwd(), 'public'),
  manifest,
  mode = 'production',
  getLoadContext,
  emitDevEvent,
  ...
  runtime
    .loadBuild()
    .then(() =>
      runtime.checksum ? runtime.ensureCompilation({ checksum: runtime.checksum }) : void 0
    )
  ...
  return async (request: Request): Promise<Response> => {
    ...
    if (assets.assets.has(url.pathname)) {
      ...
      const contentTypeHeader = contentType(url.pathname.split('.').slice(-1)[0])
...
function createRuntime({
  ...
  const loadBuild = async (): Promise<ServerBuild> => {
    ...
    if ((mode === 'production' || lastModifiedTime <= lastBuildTime) && lastBuild) {
      return lastBuild
    }
    ...
    if (mode === 'production') {
      ...
      lastRoutes = new Map(
        Object.values(newBuild.routes as Record<string, { file: string; id: string }>).map((r) => [
    ...
    const newBuild: ServerBuild = {
      entry: {
        module: await import(path.resolve(appDirectory, 'entry.server.tsx') + '?ts=' + timestamp),
      },
          ...
          [route.id]: {
            ...
            module: {
              action: async (...args) => {
                ...
                return (routeModule!.action as (...args: unknown[]) => unknown)?.(...args) || null
              },
              loader: async (...args) => {
                await ensureRouteModule()
                return (routeModule!.loader as (...args: unknown[]) => unknown)?.(...args) || null
              },
              get CatchBoundary() {
  ...
  function createBuildFromManifest(checksum: string, manifest: unknown) {
    ...
    return {
      ...
      assets: {
        ...
        routes: Object.values(serverBuild.assets.routes).reduce((acc, route) => {
          return {
            ...acc,
            [route.id]: {
              ...route,
  ...
  async function ensureCompilation({ checksum }: { checksum: string }) {
    checksum = checksum || (await buildChecksum({ appDirectory, assetsDirectory }))
    const getPlugins = () => {
      const browserRouteModulesPlugin = {
        ...
        setup(build: esbuildWasm.PluginBuild) {
          ...
          build.onLoad({ filter: /.*/, namespace: 'browser-route-modules' }, async (args) => {
            ...
            if (file) {
              ...
              if (meta) {
                const theExports = meta.exports.filter((ex) => !!browserSafeRouteExports[ex])
    ...
    if (!compilationPromise || lastModifiedTime > lastClientBuildTime) {
      ...
      const getEntryPoints = async () => {
        const entry = await findFileWithExt(path.resolve(appDirectory, 'entry.client'), [
          '.tsx',
          '.ts',
        ])
      ...
      compilationPromise = Promise.all([getEntryPoints(), ensureEsbuildInitialized()]).then(
        ([{ entryPoints }]) =>
          esbuild.build({
            absWorkingDir: cwd(),
            entryPoints: {
  ...
  return {
    ...
    async serveAssets(url: URL): Promise<Response | undefined> {
      ...
      if (url.pathname.endsWith(`/${checksum}/manifest.js`)) {
        return new Response(`window.__remixManifest=${JSON.stringify(lastBuild!.assets)};`, {
          headers: contentTypeHeader
            ? {
                'Content-Type': contentTypeHeader,
                'Cache-Control': 'public, max-age=31536000, immutable',
...
export async function loadRoutes(appDirectory: string) {
  ...
  try {
    ...
    for await (const entry of fs.walk(routesDir, { maxDepth: 1 })) {
      ...
      const index = withoutExtension === 'index' || withSlashes.endsWith('/index')
      const withoutIndex = index ? withSlashes.replace(/\/?index$/, '') : withSlashes
...
export async function writeGeneratedFile({
  ...
  const serverEntry =
    './' +
    path
      .relative(path.dirname(generatedFile), path.resolve(appDirectory, 'entry.server.tsx'))
      .replace(/\\/g, '/')
    ...
    .map(
      (route, index) =>
        `import * as route${index} from ${JSON.stringify(
          './' + path.relative(path.dirname(generatedFile), route.file).replace(/\\/g, '/')
        )};`
    )
    .join('\n')
...
export async function dev<Context>({
  generatedFile = cwd() + '/remix.gen.ts',
  assetsDirectory = path.resolve(cwd(), 'assets'),
  port = defaultPort,
  ...options
}: CommonOptions<Context>) {
  assets.settings.assetsDirectory = assetsDirectory
    ? path.resolve(assetsDirectory || path.resolve(cwd(), 'assets'))
    : undefined
  assets.settings.getChecksum = () =>
    buildChecksum({
      appDirectory: path.resolve(options.appDirectory || path.resolve(cwd(), 'app')),
...
export async function start<Context>({
  ...
  assets.settings.assetsDirectory = path.resolve(assetsDirectory || path.resolve(cwd(), 'assets'))
  assets.settings.getChecksum = () =>
    buildChecksum({
      appDirectory: path.resolve(options.appDirectory || path.resolve(cwd(), 'app')),
      assetsDirectory: assets.settings.assetsDirectory,
